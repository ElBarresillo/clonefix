...
Añadimos en la parte inferior de la pagina una
seccion donde se muestran 20 peliculas/series.
Añadimos un pequeño fondo que es la imagen del
show y ahora el boton Ver Ahora tiene la función
de mostrarnos la información del show al aparecer
un modal al darle clic.
...
const renderHero = show => {
    ...
    heroDesc.innerHTML = striptHTML(show?.summary || '').slice(0,200) + '...'
    heroPlay.onclick = () => openDetail(show.id)
}

const striptHTML = html => {
...
const openDetail = async (id) => {
    ...
    <div class="row g-4">
        ...
        <div class="col-md-8">
            ...
            <p class="text-secondary small">
                ⭐ ${show?.rating?.average ?? 'N/A'} 🎞️ Lenguaje:  ${show?.language ?? 'N/A'} 🎞️ Status: ${show?.status ?? 'N/A'}
            </p>
